<template>
  <div class="roles">
    <!-- 搜索 -->
    <el-input class="elInput" placeholder="请输入内容" v-model="search">
    ...
    <!-- 添加 -->
    <el-button type="info" round class="addUser" @click="addCase=true">添加用户</el-button>
      ...
      <el-table-column type="expand">
        <template slot-scope="scope">
          <el-row
            ...
            :key="item1.id"
          >
            <el-col :span="5" class="paddingAuto">
              ...
              >
                ...
                <el-col :span="13">
                  <el-row class="threeFl" v-for="(item3) in item2.children" :key="item3.id">
                    <el-col :span="13">
                      <el-tag type="danger">{{item3.authName}}</el-tag>
                    </el-col>
                  </el-row>
                </el-col>
              </el-row>
      ...
      <el-table-column label="操作">
        <template slot-scope="scope">
          ...
          <el-button
            size="mini"
            type="warning"
            @click="showManageCase(scope.row.children,scope.row.id)"
          >分配权限</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!-- 编辑弹框 -->
    <el-dialog title="修改信息" :visible.sync="updataCase" width="30%" :before-close="handleClose">
    ...
    <!-- 删除弹框 -->
    <el-dialog title="确认删除" :visible.sync="deleteCase" width="30%" :before-close="handleClose">
    ...
    <!-- 权限管理弹框 -->
    <el-dialog title="权限管理" :visible.sync="manageCase" width="30%" :before-close="handleClose">
      <el-tree
        :data="manageList"
        ref="tree"
        show-checkbox
...
export default {
  ...
  methods: {
    ...
    showDeleteUser(data) {
      //删除弹框
      (this.deleteId = data.id), (this.DelRoleName = data.roleName);
    ...
    searchFn() {
      //搜索
      if (this.search) {
        getRolesSearch({ id: this.search }).then(res => {
          // console.log(res)
          if (res.meta.status == 200) {
            this.initData = [
              { id: "", roleName: "", roleDesc: "", children: "" }
            ];
